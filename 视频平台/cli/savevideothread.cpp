...
//写入数据
//插帧 将以前的画面插入来同步音频时间 如果之前帧够多，插入音频时间之前的帧 否则插入之前的一帧

savevideothread::savevideothread()
{
    pic_Read=new PicInPic_Read;

    connect(pic_Read,SIGNAL(SIG_sendVideoFrame( QImage )),this,SIGNAL(SIG_sendVideoFrame( QImage )));//转发桌面
    connect(pic_Read,SIGNAL(SIG_sendPicInPic( QImage)),this,SIGNAL(SIG_sendPicInPic( QImage )));//转发画中画防止直接访问读 采集线程

    connect(pic_Read,SIGNAL(SIG_sendVideoFrameData(uint8_t* , int)),this,SLOT(Slot_writeVideoFrameData(uint8_t *, int)));//采集视频
    m_audio=new Audio_Read;
    connect(m_audio,SIGNAL(SIG_sendAudioFrameData( uint8_t*, int)),this,SLOT(Slot_writeaudioFrameData(uint8_t *, int)));//采集音频
...
void savevideothread::videoDataQuene_Input(uint8_t * buffer, int size, int64_t
                                           time)
{
...
BufferDataNode *savevideothread::videoDataQuene_get(int64_t time)
{
...
void savevideothread::audioDataQuene_Input(const uint8_t *buffer, const int &size)
{
    BufferDataNode  *node = (BufferDataNode*)malloc(sizeof(BufferDataNode));
    node->bufferSize = size;
    node->buffer = (uint8_t*)buffer;
    m_audioMutex.lock();
    m_audioDataList.append(node); //队列加入数据
    m_audioMutex.unlock();
}
BufferDataNode  * savevideothread::audioDataQuene_get()
{
    BufferDataNode  *node = NULL;
    m_audioMutex.lock();
...
void savevideothread::run()
{
    ...
    audio_st.next_pts, audio_st.enc->time_base) <= 0)) )//每次将较小的帧先插入
    {
    ...
    || ( have_video && ( m_videoDataList.size() == 0) ))//如果都非空 还要根据pts来进行插入帧
        break;
     }
        }
    while(1)
    ...
    /* Close each codec. */
    if (m_av_format.hasCamera||m_av_format.hasDesk)
        close_stream(oc, &video_st);
    if (m_av_format.hasAudio)
...
bool savevideothread::write_audio_frame(AVFormatContext *oc, OutputStream *ost)
{
    //qDebug() << __func__;
    AVCodecContext *c;
    c = audio_st.enc;
...
bool savevideothread::write_video_frame(AVFormatContext *oc, OutputStream *ost,
                                        double time)
{
   // qDebug() << __func__;
...
int savevideothread::write_frame(AVFormatContext *fmt_ctx, AVCodecContext
                                 *c,AVStream *st, AVFrame *frame ,int64_t & pts , OutputStream *ost ) //写帧
{
    int ret;
...
int savevideothread::write_frame(AVFormatContext *fmt_ctx, AVCodecContext *c,
                                 AVStream *st, AVFrame *frame)
{
    //将帧数据写入包中  帧组成流
    //帧（Frame）可以被封装成一个个的数据包（Packet），这些数据包进而可以组成Stream流进行传输
    int ret;
    if(!frame) return 0;

    // send the frame to the encoder
    ret = avcodec_send_frame(c, frame);//发送帧
    ...
    while (ret >= 0) {
        ...
        ret = avcodec_receive_packet(c, &pkt);//接收
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
            break;
        else if (ret < 0) {
            fprintf(stderr, "Error encoding a frame\n");
        ...
        /* rescale output packet timestamp values from codec to stream timebase */
        av_packet_rescale_ts(&pkt, c->time_base, st->time_base);//写帧 音频 和视频的pts
        pkt.stream_index = st->index;

        /* Write the compressed frame to the media file. */

        ret = av_interleaved_write_frame(fmt_ctx, &pkt);//写帧入文件
        av_packet_unref(&pkt);
        if (ret < 0) {
            fprintf(stderr, "Error while writing output packet\n");
            exit(1);
...
void savevideothread::Slot_writeaudioFrameData(uint8_t *picture_buf, int buffer_size){

    //视频如果没有 , 丢弃帧  拉齐视音频时间
    if( (m_av_format.hasCamera || m_av_format.hasDesk)&& m_videoBeginFlag )
    {
        free( picture_buf );//开始录制才采集音频
        return;
    }
    ...
    //    rational.num = 1;
    //    rational.den = c->sample_rate;
    //    audio_st.frame->pts = av_rescale_q(audio_st.samples_count, rational, c->time_base);//计算时间用于 同步
    //    audio_st.samples_count += audio_st.frame->nb_samples;
       //write_frame(oc, audio_st.enc, audio_st.st, audio_st.frame);

}

void savevideothread::Slot_writeVideoFrameData(uint8_t *picture_buf, int buffer_size)
{
    //记录第一次时间
    ...
    //写入视频帧picture_buf -> ost->framebuffer
    //    memcpy( video_st.frameBuffer  , picture_buf , buffer_size );
    ...
    //    write_frame(oc, video_st.enc,video_st.st,video_st.frame);
    //    //av_free(picture_buf);//结束完回收

}

void savevideothread::slot_openvideo()
{
    if(m_av_format.hasCamera||m_av_format.hasDesk){
        m_videoBeginFlag = true; //来开始采集音频 并采集起始时间
        pic_Read->slot_openVideo();
    }
    if(m_av_format.hasAudio)
...
void savevideothread::close_stream(AVFormatContext *oc, OutputStream *ost)
{
...
void savevideothread::slot_closevideo()
{
    if (m_av_format.hasCamera||m_av_format.hasDesk)
        pic_Read->slot_closeVideo();
    if(m_av_format.hasAudio)
...
void savevideothread::add_audio_stream(OutputStream *ost, AVFormatContext *oc,
                                       AVCodec **codec,
...
/* Add an output stream. */
void savevideothread::add_video_stream(OutputStream *ost, AVFormatContext *oc,
                                       AVCodec **codec,
                                       enum AVCodecID codec_id)
{
    AVCodecContext *c;
    ...
    *codec = avcodec_find_encoder(codec_id);
    ...
    c->bit_rate = m_av_format.videoBitRate;
    /* Resolution must be a multiple of two. */
    c->width    = m_av_format.width;
    c->height   =  m_av_format.height;
    /* timebase: This is the fundamental unit of time (in seconds) in terms
         * of which frame timestamps are represented. For fixed-fps content,
         * timebase should be 1/framerate and timestamp increments should be
         * identical to 1. */
    ost->st->time_base = { 1, m_av_format.frame_rate};//获取帧率 15 帧 1s15个画面
...
void savevideothread::open_audio(AVFormatContext *oc, AVCodec *codec, OutputStream *ost)
{
    AVCodecContext *c;
    int nb_samples;
    int ret;
...
void savevideothread::open_video(AVFormatContext *oc, AVCodec *codec, OutputStream *ost)
{
    int ret;
...
void savevideothread::setinfo(STRU_AV_FORMAT &av_format)
{
    m_av_format=av_format;
    char filename[1024]="";
    std::string path=m_av_format.filename.toStdString();
    sprintf(filename,"%s",path.c_str());//提取文件路径
    ...
    if (have_video)
        open_video(oc, video_codec, &video_st);//申请空间 打开编码器

    if (have_audio)
        open_audio(oc, audio_codec, &audio_st);

    av_dump_format(oc, 0, filename, 1); //输出；流
    ...
    ret = avformat_write_header(oc,NULL);//写入头 根据头来写入帧数据
    if (ret < 0) {
