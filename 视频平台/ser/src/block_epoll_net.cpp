...
bool Block_Epoll_Net::InitNet(int port, void (*recv_callback)(int, char *, int))
{
    ...
    InitThreadPool();
    tcp_redis=new CRedis;
    char keys[1024]="";
    sprintf(keys,"Over_due_clientfd");
    if(tcp_redis->isExist(keys)) tcp_redis->removeKey(keys);
    ...
    //sprintf(keybuf,"",);
    if(tcp_redis->isExist(keybuf))  tcp_redis->removeKey(keybuf);
    if(tcp_redis->isExist(keybuf2)) tcp_redis->removeKey(keybuf2);
    //creat a tcp socket
...
void Block_Epoll_Net::EventLoop()
{
    ...
    while (1) {
       ...
       //tcp_redis->getList(keys,remove);
        if(remove.size()!=0){//检查到过期的键值对

            string op=remove.front();
            cout<<"check_Not_alive_clientfd:"<<op<<endl;
            remove.pop_front();
            tcp_redis->Remvals(keys,op);
            myevent_s* eva =nullptr;
            m_mapSockfdToEvent.find(stoi(op),eva);

            eva->eventdel();
            close(eva->fd);
            //回收event结构
            //pthis->m_mapSockfdToEvent.erase( ev->fd );
            m_mapSockfdToEvent.erase( eva->fd );
            delete eva;
...
void Block_Epoll_Net::accept_event()
{
    ...
    tcp_redis->appendOneToList(keybuf,to_string(clientfd));
    sprintf(keybuf,"clientfd:%d",clientfd);
    tcp_redis->setex(keybuf,"1",_CHECK_ALIVE_TIME);
...
void* Block_Epoll_Net::recv_task(void* arg)
{
    ...
    pthis->tcp_redis->removeKey(keybuf);
    sprintf(keybuf,"m_epoll_fd:1");
    pthis->tcp_redis->Remvals(keybuf,to_string(ev->fd));
    char keys[1024]="";
    sprintf(keys,"Over_due_clientfd");
    pthis->tcp_redis->Remvals(keys,to_string(ev->fd));
    //回收event结构
    //pthis->m_mapSockfdToEvent.erase( ev->fd );
    ev->eventdel();
    close(ev->fd);
